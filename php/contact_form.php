...
if($_POST)
{
	...
	$to_email = "chad.lyell@protonmail.com";
	...
	if(!$send_mail)
	{
		$output = json_encode(array('type'=>'error', 'text' => 'There was an error while sending the email.'));
		die($output);
	}
	else
	{
		$output = json_encode(array('type'=>'message', 'text' => 'Thanks for sending an email, ' . $sender_name . '. <br> I will reply as fast as possible!'));
		die($output);
	}
